...
from utils.utils import get_channel_by_id, get_channel_by_ids, get_subscriber_role_by_channel
...
async def quests(client):
    ...
    while True:
                ...
                for channelname, channel, _ in channels:
                    ...
                    if role:
                        ...
                        async for message in client.logs_from(channel, limit=1):
                            if message.embeds != [] and message.embeds[0].description.contains(" ".join(desc.split()[1:])):
                                return
        ...
        elif gmtime(time()).tm_mday in [23, 29]:
